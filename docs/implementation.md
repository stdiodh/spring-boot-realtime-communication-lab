...
5. `realtime-demo.html`에서 채팅처럼 메시지를 보내고 다시 받는지 확인합니다.
...
4. 채팅 영역에 같은 메시지가 말풍선처럼 다시 찍히는지 봅니다.
5. 가능하면 브라우저 탭을 두 개 열어서 한 탭에서 보낸 메시지가 다른 탭에도 바로 보이는지 확인합니다.
...
4. 채팅 영역과 이벤트 로그에 실시간으로 메시지가 다시 표시되는지 확인합니다.
5. 브라우저 탭 두 개를 열어 cross-tab 수신도 확인해봅니다.
